...
const select = async (filter: EventsContestantsProperties): Promise<
  EventsContestantsProperties
> => {
  const doc = await findOne<EventsContestantsDocument>("events_contestants", filter)
...
> => {
  ...
  const doc = await insert<EventsContestantsDocument>("events_contestants", data)
  const contestant = await populate(doc)
  return contestant
}
...
> => {
  ...
  const doc = await updateOne<EventsContestantsDocument>(
    "events_contestants", filter, data
  )
  const contestant = await populate(doc)
  return contestant
}
...
> => {
  ...
  const docs = await updateMany<EventsContestantsDocument>("events_contestants", data)
  const contestants = await Promise.all(docs.map((doc) => populate(doc)))
  return contestants
}

const removeOne = async ({ id }): Promise<true> => {
  await destroyOne<EventsContestantsDocument>("events_contestants", { id })
  return true
}

const removeMany = async (filters: { id: string }[]): Promise<true> => {
  const filter = { id: { $in: filters.map(({ id }) => id) } }
  await destroyMany<any>("events_contestants", filter)
  return true
}
